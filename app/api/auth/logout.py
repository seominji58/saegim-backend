...
class LogoutService:
    ...
    def log_logout_attempt(self, user_id: str, success: bool, details: str = "") -> None:
        """로그아웃 시도 기록"""
        try:
    ...
    try:
        ...
        if user and user.account_type == "social" and user.provider == "google":
            ...
            if oauth_token and oauth_token.access_token:
                if not await logout_service.revoke_google_token(oauth_token.access_token):
        ...
        return BaseResponse(
            data={
                ...
                "provider": user.provider if user and user.account_type == "social" else None,
