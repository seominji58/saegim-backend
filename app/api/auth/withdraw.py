...
async def withdraw_account(
    ...
    try:
        ...
        user = (
            db.query(User)
            .filter(User.id == current_user_id, User.deleted_at is None)
            .first()
        )
        ...
        if user.account_type == "email":
            ...
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="지원하지 않는 계정 타입입니다."
            )
        ...
        # FCM 토큰 삭제
        db.execute(delete(FCMToken).where(FCMToken.user_id == current_user_id))

        # 알림 설정 삭제
        db.execute(
            delete(NotificationSettings).where(
                NotificationSettings.user_id == current_user_id
            )
        )
        ...
        db.execute(
            delete(NotificationHistory).where(
                NotificationHistory.user_id == current_user_id
            )
        )

        # 감정 통계 삭제
        db.execute(delete(EmotionStats).where(EmotionStats.user_id == current_user_id))

        # AI 사용 로그 삭제
        db.execute(delete(AIUsageLog).where(AIUsageLog.user_id == current_user_id))

        # 알림 삭제
        db.execute(delete(Notification).where(Notification.user_id == current_user_id))

        # OAuth 토큰 삭제
        db.execute(delete(OAuthToken).where(OAuthToken.user_id == current_user_id))

        # 비밀번호 재설정 토큰 삭제
        db.execute(
            delete(PasswordResetToken).where(
                PasswordResetToken.user_id == current_user_id
            )
        )
