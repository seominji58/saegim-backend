...
async def restore_account(
    ...
    try:
        ...
        user = (
            db.query(User)
            .filter(User.email == request.email, User.deleted_at.is_not(None))
            .first()
        )

        if not user:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="탈퇴된 계정을 찾을 수 없습니다."
            )
        ...
        if not verification:
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED, detail="유효하지 않은 인증 코드입니다."
            )
