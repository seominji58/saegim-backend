...
async def get_profile(
    ...
    try:
        ...
        return BaseResponse(data=profile_data, message="프로필 정보 조회 성공")
...
async def update_profile(
    ...
    try:
        ...
        if request.nickname != current_user.nickname:
            ...
            if existing_user:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST, detail="이미 사용 중인 닉네임입니다."
                )
        ...
        return BaseResponse(data=profile_data, message="프로필 업데이트 성공")
...
async def check_nickname_availability(
    ...
    try:
        ...
        if nickname == current_user.nickname:
            data = {"available": True, "message": "현재 사용 중인 닉네임입니다."}
        else:
