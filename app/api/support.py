...
class SupportInquiryRequest(BaseModel):
    ...
    image_data: Optional[str] = Field(default=None, description="Base64 인코딩된 이미지 데이터")
    image_filename: Optional[str] = Field(default=None, description="이미지 파일명")
    image_type: Optional[str] = Field(default=None, description="이미지 MIME 타입")
...
async def create_support_inquiry(
    ...
    try:
        ...
        inquiry_id = f"INQ_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{str(current_user.id)[:8]}"
